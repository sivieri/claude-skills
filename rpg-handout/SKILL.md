...
- **Keep backgrounds vector — avoid `mix-blend-mode`, `filter`, and `backdrop-filter`.** A blend mode or CSS filter anywhere on the page forces Chrome's "Print to PDF" to **flatten the whole page into one lossless full-page bitmap**: the PDF becomes slow to *open* (the viewer must decode that image every time — felt as lag in Preview), and any later recompression shifts its colour (e.g. Ghostscript strips the colour profile → renders darker). Plain `linear-gradient`/`radial-gradient` stacks, `box-shadow`, `clip-path`, and `text-shadow` all stay vector and are fine. To get a "multiply"-style warm wash, layer a **normal-composited** translucent gradient (e.g. `rgba(150,120,60,.06)` → `rgba(110,80,35,.20)`) instead of a blend mode. Quick check: in the exported PDF the body text should still be **selectable** (= vector), not part of an image. Don't post-process with Ghostscript to shrink these — fix the source instead.
